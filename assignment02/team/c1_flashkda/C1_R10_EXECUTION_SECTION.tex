...
\begin{tabular}{l r r r r}
\toprule
variant & block & registers/thread & shared config (B) & achieved occupancy \\
\midrule
default & 192 & 66--74 & 200704 & 9.36--9.38\% \\
pair-packed direct & 192 & 78--92 & 200704 & 9.37--9.38\% \\
\bottomrule
\end{tabular}
\caption{NCU basic set 的 recurrence resource 结果；范围来自不同 state I/O 实例化。}
\end{table}

direct 版本的寄存器数不是下降，而是增加约 5--26 registers/thread；shared
memory 配置完全没有变化，因为本轮没有删除 union 中预留的 output storage。
