...
端到端收益。R19 的 full 8-block register-state cache 通过 layout map 和七个
end-to-end exactness case，但在独立 CUDA 13.0/cu130 的 B300 ABBA 对照中慢
14.92\%。随后加入循环外 preload、value-major fragment、warp sync 和 early
output，最终组合通过 576 项 exactness；题面 H96 BF16 state 的独立 ABBA 从
1.746072 ms 降到 1.643856 ms，延迟降低 5.854\%（1.06218 倍速度）。因此
...
节点上测得的官方非 split 路径。早期 cu129 harness 的固定形状 BF16-state
baseline 约为 \(1.03\,\mathrm{ms}\)；R19 的隔离 cu130 harness 则以其同一
job 的 1.743280 ms control 为准，两者不横向比较。正式 benchmark 使用 CUDA
event，warmup/iters/repeats 均在对应轮次记录；profiler 时间只用于结构解释。
...
\begin{enumerate}
  ...
  \item 局部替换 atom、value-split 和多种输出改写没有稳定收益，但 R19 的
  完整 state cache、循环外 preload、value-major 和同步改写组合有正结果：
  H96 BF16 延迟降低 5.854\%，H96 no-state 降低 12.971\%。保持 CHUNK=16
  和 SM80 MMA 仍有优化空间；不能把早期负结果外推为不可能改进。
...
下表汇总 R3--R19 的完整 output 或结构性候选。不同轮次的绝对时间受节点
...
R19 & full 8-block register-state cache & map 0 mismatch；7/7 output/state exact & cu130 ABBA：2.003360 vs 1.743280 ms，延迟增加 14.92\%\\
R19 & early-output only & 7/7 output/state exact & cu130 ABBA：1.735928 vs 1.756552 ms，延迟降低 1.174\%；后续组合复测\\
R19 & reg5 / value-major cache controls & reg5/vm8/vm5 均 7/7 exact；VM map 0 mismatch & 延迟分别增加 2.153\% / 4.095\% / 10.393\%\\
R19 & preload + value-major + warp sync + early output & 576 tests exact；value-major arithmetic probe 0 mismatch & job 25286：H96 BF16 state 降低 5.854\%，六个 case 测得略快或明显快；小于 1\% 的两项不主张稳健加速\\
\bottomrule
\caption{主要挑战路线的结果分类。}
\end{longtable}
...
同源复测，属于可重复的持平；R15--R19 中 job 25286 的最终组合已完成独立长轮次与多 shape 复测；题面 H=96 BF16 state 降低 5.854\%，但 FP32 state 与多序列 varlen 的收益较小。
...
\begin{itemize}
  \item NVIDIA 的
        ...
        性能保证。
  \item
        \href{https://github.com/Int21-AI/KDA-B200}{KDA-B200} 与
        \href{https://github.com/inclusionAI/cuLA}{cuLA} 展示了不同路线：cuLA
        的 SM100 路径使用 Blackwell-native tcgen05/TMEM；KDA-B200 的当前
        recurrence 仍含 SM80-style \code{mma\_sync}，并以 shape specialization
        和部分 register-state cache 优化局部数据驻留。后者以
        \code{movmatrix} 转换 cached fragment direction，正是 R19 要独立验证
        而非照抄的设计空间。cuLA 的 SM100 fwd\_o 还把 chunk=64、256 threads
        和 persistent/non-persistent stage 作为显式设计参数。这些资料说明
        state layout 与 dispatch 同样关键，但 B200/FLA/CUPTI 口径不能直接替代
        本题 B300 baseline。
  \item
...
Dynamic Shared Memory Per Block 为 98,432 B（约 96.125 KiB；不是每 SM 的
200,704 B configuration），NCU 观测到约 66--74 registers/thread、
约 9.37\% achieved occupancy。SM103 上仍使用 SM90 TMA descriptor 和 HMMA，
是为了保持经过验证的 16\(\times\)16 Neumann/state/layout ABI，而不是遗漏
了 Blackwell 编译目标。R19 表明，在保持该数值算法的同时改变状态驻留和
同步组织也能取得正收益；tcgen05/TMEM 仍是需要独立验证的另一条路线。

\begin{decision}{基于公开资料的工程判断}
R15--R19 证明多数低风险的局部改动已经接近其收益上限：workspace 重算会退化，
DSM primitive 会退化，PDL 合成 probe 的收益无法传递到当前 K1/K2 临界路径；
R19 的直接 full cache 会慢化，但加入 preload、value-major、warp sync 和 early output
后，在题面形状取得 5.854\% 延迟降低。这里的编译
register 变化只与慢化相关，不能在 H=96 的 CTA 数下直接断言 occupancy 因果；
job 25286 已将早提交与其它调度因素组合并完成独立复测；其题面形状延迟降低 5.854\%，但 FP32 state 与 varlen 差异小于 1\%。若继续追求更高收益，可在独立 SM103a
kernel 中同时重做 tcgen05/TMEM 的 tile、persistent/reusable-workspace 路由，
或先重排 state fragment direction 以降低 cache hit 的转换与寄存器成本；不能
把 full-cache R19 的负结果外推为所有 partial-cache 设计均不可行。若目标是
完成 C1，当前 HMMA baseline 加上这些正反证据已足以回答“为什么官方没有直接
换 tcgen05”。
\end{decision}
...
加速。R19 最终组合已经在 576 项 correctness tests 和六个 benchmark case 上通过；
题面 H=96 BF16 state 的 CUDA-event 延迟从 1.746072 ms 降至 1.643856 ms，
降低 5.854\%。收益依赖 state mode 与序列组织：FP32 state 为 0.775\%，
8-sequence varlen 为 0.960\%。full register-state cache 单独使用仍慢约 14.92\%，
说明缓存布局、初始化与同步组织需要共同测量；消融尚不能证明每一项都是必要条件。

因此，官方停在 SM80 是一个有工程依据的决定，而不是尚未尝试 tcgen05 的遗漏。
如果目标是继续追求超过 10\%，应重做 state layout/fragment direction，并结合
K1/K2 workspace fusion、CHUNK+rescale 或独立 SM100 tile 设计，接受较高的实现和
数值风险；如果目标是完成 C1，当前证据已足以支持“SM80 v1 保持、SM100 v2 暂缓”
的结论。
