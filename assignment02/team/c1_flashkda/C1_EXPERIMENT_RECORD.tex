...
\begin{decision}{R2.1 可以和不可以得出的结论}
\textbf{可以：} 5090 的 strict mismatch 是可重复的；它在 \(T=16\) 的单
...
\textbf{不可以：} 这还没有证明 5090 kernel 算法错误，更不能从两张不同设备
...
保留 job 23441 的 PATH 失败。这些均为小文件，将随本轮 commit 进入 git。
\end{record}
...
\code{AttributeError} 结束；不将其静默删除。
...
也不能声称会有端到端收益。下一次代码改动必须是隔离的 compile-time prototype，
不能污染已验证的基线路径。
\end{record}
