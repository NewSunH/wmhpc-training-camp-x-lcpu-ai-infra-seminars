% C1 FlashKDA 第一轮实验记录
% 编译：latexmk -xelatex -interaction=nonstopmode C1_FIRST_ROUND_RECORD.tex
...
\begin{document}
\begin{center}
  {\LARGE\bfseries C1：FlashKDA 第一轮实验全记录}\\[6pt]
  {\sffamily 作业 2 Team / 可审计实验日志（不是答辩版本）}
\end{center}
\vspace{0.5em}\hrule\vspace{1em}

\begin{record}{文档范围与使用说明}
本文事无巨细地记录 C1 第一轮：题目与源码阅读、两套 Slurm 集群的接入和
环境构建、FlashKDA extension 的编译、官方正确性和 benchmark、NCU/SASS
检查、SM100 tcgen05 正反例、CHUNK 数值范围 probe、state 精度 probe。
文中的命令均来自实际执行过程；为避免把个人用户名、IP 和绝对目录写进
