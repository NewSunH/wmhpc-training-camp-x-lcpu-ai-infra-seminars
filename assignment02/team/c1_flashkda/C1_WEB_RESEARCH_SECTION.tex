...
{\code{flash\_kda\_cuda.cu}} 可见，它并非把所有 recurrence 都替换为
\code{tcgen05}：\code{warp\_recurrence\_value\_pair\_bf16} 的投影和 state
update 仍显式调用 \code{mma\_sync\_m16n8k16\_bf16}。更关键的是，该函数的
模板参数 \code{RegisterStateKeys} 与 \code{CacheProjectionState} 允许每个
warp 先将一部分 state key block 读入 \code{uint32\_t} register fragment，后续
chunk 的 projection 对 cache hit 通过 \code{movmatrix\_trans\_b16} 转成所需
B-fragment 方向；其 launch-time specialization 按 fixed/varlen、value slice 和
compute-warp 形态选择 1--7 个 cached key blocks。这个公开实现给 R19 的
``partial register-state cache'' 提供了设计线索，但两者的 layout、同步、设备和
benchmark 口径不同；本项目没有复制该实现，也不能把其公开成绩当作 R19 的性能
结果。
...
\begin{enumerate}
  ...
  \item K2 recurrence 固定输入 stage=3、output stage=2、非 split block
        ...
        负责 load/store。NCU 的 Dynamic Shared Memory Per Block 在本题 H=96
        形状为 98,432 B（约 96.125 KiB）；200,704 B 是 Shared Memory
        Configuration/SM，不能当作单个 CTA 的 dynamic shared-memory，
...
包含两类不同路线：一类采用 Blackwell-native tcgen05/TMEM、shape
specialization、persistent 调度或 reusable workspace；另一类仍使用 SM80-style
MMA，但以 shape specialization、部分寄存器 state cache 和流水线组织降低
recurrence 的局部开销。前者说明完整 SM103 专版的潜在上限，后者说明
``保留 HMMA ABI、重构 state 数据驻留'' 也是需要实测的独立假设；两者都不能
替代本题的同源 B300 对照。当前默认 baseline 保持不变，R15/R17/R18 的
opt-in 分支只作为可复核的负向或边界证据。
\end{decision}

\paragraph{R19 后续实测更正。}
上述公开资料筛选写于 R19 完成前。job 25286 已在本题 B300/CUDA-event 口径下
实证：保持 CHUNK=16 和 SM80 MMA，组合 state cache、value-major、preload 与
同步改写，H96 BF16 state 延迟降低 5.854\%（1.06218 倍速度），576 项 exactness
通过。因此“要超过 baseline 就必须迁移 tcgen05”的强判断不成立。完整原始数据
与接入边界见 R19；公开外部性能仍不代替本地同卡对照。
