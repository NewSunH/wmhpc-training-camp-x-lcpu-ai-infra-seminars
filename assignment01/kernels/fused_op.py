...
def scale(x: torch.Tensor) -> torch.Tensor:
    ...
    scale_kernel[grid](x, z, n, BLOCK_SIZE=BLOCK_SIZE) # pyright: ignore[reportArgumentType]
    return z
...
@triton.jit
def fused_kernel(x_ptr, z_ptr, n, a:float, b:float, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
    offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offsets < n
    x = tl.load(x_ptr + offsets, mask=mask, other=0.0)
...
def fused(x: torch.Tensor, a: float, b: float) -> torch.Tensor:
    ...
    BLOCK_SIZE = 1024
    grid = (triton.cdiv(n, BLOCK_SIZE),)
    fused_kernel[grid](x, z, n, a, b, BLOCK_SIZE=BLOCK_SIZE)  # TODO：把 a、b 传进去 # pyright: ignore[reportArgumentType]
    return z
