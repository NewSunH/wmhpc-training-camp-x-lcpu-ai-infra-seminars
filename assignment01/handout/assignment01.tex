...
实测 interleaved \underline{\hspace{3em}} ms、contiguous \underline{\hspace{3em}} ms，比值约 \underline{\hspace{2em}}（跑 \texttt{make run/m3\_simt/03\_reduce} 记录）。

解释：两版每 block 都是 8 轮配对加法、加法次数完全相同，差距来自活跃线程在 warp 里的分布。interleaved 版每轮干活的 lane 隔 $2s$ 取一个（\texttt{tid \% (2*s) == 0}），散布在全部 8 个 warp 里——每轮每个 warp 都要照常取指、发射，但多数 lane 在空转，SIMT 按 warp 计的开销一点没省，还要每轮对运行期步长做一次取模。contiguous 版活跃线程挤在低编号一端（\texttt{tid < s}），活跃 warp 数每轮减半（$s=128$ 时 4 个、$s=64$ 时 2 个、$s \le 32$ 后只剩 1 个），其余 warp 一次分支判断就退出，浪费的发射槽位少得多，配对元素也始终相邻访存。两版每轮都要全 block 的 \texttt{\_\_syncthreads}，这部分开销相同、不构成差异；真正拉开的差距是“有效工作 / 发射指令”的比率，interleaved 因此慢出一倍以上。
...
实测 naive \underline{\hspace{3em}} ms、priv \underline{\hspace{3em}} ms，提速约 \underline{\hspace{2em}}x（跑 \texttt{make run/m4\_memory/04\_histogram\_priv} 记录；参考：A100 实测 149x、V100 实测 86x）。
...
(a) 报错形如 \texttt{CUDA error cudaErrorNoKernelImageForDevice at ...: no kernel image is available for execution on the device}（error 209）。可执行文件里只有 \texttt{sm\_90} 的 SASS，而 5090 是 compute capability 12.0——SASS 与具体架构绑定，fatbin 里既没有匹配的 image 也没有 PTX 兜底，运行时无从加载。

(b) 能正常运行。这个版本只带 \texttt{compute\_75} 的 PTX，驱动在首次加载时把它 JIT 编译成 \texttt{sm\_120} 的 SASS 再执行。即 PTX 到机器码的编译发生在运行时、由 CUDA driver 完成（Guide 1.3.2、2.1.1）。
...
\end{lstlisting}
        ...
        \textcolor{gray}{（待补：BS=8/16/32 实测数据——各用时 \underline{\hspace{2em}} ms / \underline{\hspace{2em}} GFLOPS）}
...
\end{lstlisting}
        ...
        \textcolor{gray}{（待补：各 tile 配置实测数据；最优配置 \underline{\hspace{3em}}，约 \underline{\hspace{2em}} TFLOPS）}
...
\end{lstlisting}
        ...
        \textcolor{gray}{（待补：各 block\_M/block\_N/block\_K/num\_stages 配置实测数据；最优配置 \underline{\hspace{3em}}，约 \underline{\hspace{2em}} TFLOPS）}
\end{enumerate}

性能差距的原因（以 A100 参考数据为例：naive CUDA 约 2.4 TFLOPS、Triton 约 125、TileLang 约 118、cuBLAS 约 173）：

naive CUDA 版是 fp32 标量乘加，每个输出元素要把 A 的一行、B 的一列从 global memory 反复读 K 遍，算术强度极低、纯带宽受限，而且完全用不到 tensor core，所以只有个位数 TFLOPS；BS 只改线程块形状、不改变数据复用结构，因此收益有限。Triton 与 TileLang 版把数据按 tile 搬进 shared memory 复用、用 fp16 tensor core 执行 \texttt{tl.dot}/\texttt{T.gemm}，性能提升两个数量级；cuBLAS 是厂商针对各架构手写调优的库，还能再上一个台阶。

TileLang 控制粒度更细却略慢，是因为“细粒度”不等于“自动变快”：TileLang 把 layout、shared 分配、流水结构显式交给用户，但最终性能取决于编译器为这些结构生成的代码质量（layout inference、指令调度、tensor core 指令选择、流水展开）以及对目标架构的调优成熟度。本仓库固定的 tilelang 0.1.12 仍在快速迭代，这些后端优化不如更成熟的 Triton。注意两组数字都只是各自 \texttt{bench()} 里几组默认配置的最优值，不代表工具的性能上限。
