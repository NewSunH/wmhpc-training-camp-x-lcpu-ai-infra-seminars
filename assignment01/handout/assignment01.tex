...
\begin{center}\small
  \begin{tabular}{@{}lll@{}}
    ...
    项目                      & 笔记本                                & infra5090               \\
    \midrule
    型号 / compute capability & NVIDIA GeForce RTX 4060 Laptop GPU & NVIDIA GeForce RTX 5090 \\
    SM 数量                   & 24                                 & 170                     \\
    warp 大小                 & 32                                 & 32                      \\
    shared memory / block   & 49152                              & 49152                   \\
    最大常驻线程 / SM             & 1536                               & 1536                    \\
    显存总量                    & 8186822656                         & 33668857856             \\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{enumerate}[label=(\alph*)]
  \item 一块标称 100 TFLOPS 的 GPU，执行单条指令的延迟一定低于 5 GHz 的 CPU。（错）
  \item HBM 的“高带宽”指大块连续访问时的吞吐，零散的随机访问达不到标称值。（对）
  \item 严格串行的迭代算法（每步依赖上一步的结果），即使换一块算力更强的 GPU 也快不了多少。（对）
  \item “算力 1000 TFLOPS”意味着每次运算的延迟是 $10^{-15}$ 秒。（错）
\end{enumerate}

\prob{1.2}{CONCEPT}
Session 1 讲座里提过“N 方过百万”这个例子。总计算量 $10^{12}$ FLOP 在当代 GPU 上的运算时间大概是毫秒级，那为什么一个严格在线的串行算法仍然做不到几秒内跑完？（从“延迟”和“吞吐”的角度考虑）

串行算法需要非常低的延迟。GPU 通过大量并行实现延迟降低，但是一个串行算法无法进行并行优化。因此这样的算法仍然无法快速计算。

\prob{1.3}{CONCEPT}
补全下表（thread 一行已填好作为示例）

\begin{center}\small
  \begin{tabular}{@{}L{5.2em}L{9em}L{7.5em}L{7.5em}L{8em}@{}}
    \toprule
    执行层次        & 软件含义                                 & 对应硬件                    & 直接可用的存储                        & 同步与通信手段                        \\
    \midrule
    thread      & kernel 的最小执行单位                       & 计算单元上的一个 lane           & 自己的寄存器                         & （自身天然有序）                       \\
    warp        & 32 个 thread                          & 一个 SM 上的 SIMD/SIMT 执行单元 & warp 内 thread 的寄存器             & warp shuffle                   \\
    block / CTA & thread 的协作组                          & 一个 SM 上驻留的一组 thread     & 寄存器 + shared memory + L1 cache & shared memory                  \\
    grid        & kernel 一次 launch 的全部 thread block 集合 & 整个 GPU 上的所有 SM          & global memory                  & kernel launch 边界同步，cuda stream \\
    \bottomrule
  \end{tabular}
\end{center}
...
SIMD：一指令多数据；SIMT：一指令多线程。二者变成抽象不同。SIMD 是显式的向量化模型，一条指令直接作用于多个数据元素，例如 CPU 的 AVX 指令通过一个向量寄存器同时处理多个浮点数，程序员需要显式管理数据打包和向量宽度。SIMT 是 NVIDIA GPU 采用的执行模型，程序员以线程（thread）为单位编写代码，每个线程拥有自己的寄存器和逻辑视角，但硬件会将 32 个线程组成一个 warp，以类似 SIMD 的方式执行同一条指令。因此，SIMT 可以看作是在 SIMD 硬件之上提供了线程级抽象，降低了并行编程复杂度。

错误。Volta 架构（SM 7.0）引入 Independent Thread Scheduling，使每个线程拥有独立的 program counter 和调用栈，相比之前 warp 共享 PC 的模型，在同步、线程独立退出和细粒度控制方面更加灵活。但 GPU 的实际执行资源仍以 warp 为基本调度单位，一个 warp 内的线程仍共享指令发射和计算资源。如果同一个 warp 内不同线程进入不同分支路径，硬件仍需要分别执行不同路径，导致执行效率下降。因此 branch divergence 仍然存在性能代价，只是 Volta 之后这种代价的表现形式和同步行为有所改善，而不是完全消失。
...
铺满 grid 配置：16384 blocks x 256 threads
...
\begin{center}\small
  \begin{tabular}{@{}lll@{}}
    ...
    \midrule
    CPU 单线程                  & 10.094  & 2.41    \\
    GPU \texttt{<<<1,1>>>}   & 138.423 & 33.00   \\
    GPU \texttt{<<<1,256>>>} & 2.393   & 0.57    \\
    GPU 铺满 grid              & 0.023   & 0.01    \\
    \bottomrule
  \end{tabular}
\end{center}

GPU 频率更低，单线程没有复杂的乱序 pipeline，需要大量等待内存。GPU 的加速不在于单线程计算能力更强，而是有更好的并行处理能力。
...
\begin{enumerate}[label=(\alph*)]
  \item 在 GPU 上执行、由 CPU 侧启动的 kernel 函数。\texttt{\_\_global\_\_}
  \item 只会被 kernel 调用的辅助函数。\texttt{\_\_device\_\_}
  \item host 和 device 代码都要调用的小工具函数。\texttt{\_\_host\_\_ \_\_device\_\_}
  \item 整个 kernel 运行期间不变、所有线程都要读的系数表。\texttt{\_\_constant\_\_}
  \item block 内线程共享的暂存数组。\texttt{\_\_shared\_\_}
\end{enumerate}
...
(a)~kernel 启动之后，需要把 cpu 中的数据 copy 到 GPU；CPU 读取结果之前需要把 GPU 数据copy 到 CPU，给子需要一个同步。(b)~自动更快，可能是编译器可以优化掉在 GPU 数据传输的一些时机，会比手动 malloc 快一些。

\prob{2.4}{CONCEPT}
判断对错，可以顺带补一句理由。
\begin{enumerate}[label=(\alph*)]
  \item \texttt{vectorAdd<<<...>>>(...)} 这条语句返回时，kernel 一定已经执行完毕。（错）
  \item 同一个 stream 里，\texttt{cudaMemcpy}（device 到 host）会等它前面的 kernel 全部完成后才开始拷贝。（对）
  \item kernel 内部的非法访存，会在启动语句处同步地报出来。（错）
\end{enumerate}
...
(a)~执行顺序是 GPU 内的调度器决定的，不保证顺序，(b)~不可以，因为这个顺序是不确定的。每个 block 相互独立是 scalable programming model 的基础。
...
\texttt{linear\_id = threadIdx.x + \\
  threadIdx.y * blockDim.x + \\
  threadIdx.z * blockDim.x * blockDim.y
}

\begin{enumerate}[label=(\alph*)]
  \item \texttt{threadIdx = (3, 5, 0)} 的线性编号是 43。第2个warp的第12个lane。
  \item 这个 block 一共占2个 warp。
  \item 若 \texttt{blockDim = (33, 1, 1)}，占几2个 warp。第二个 warp只有一个有效线程。
\end{enumerate}
...
warp 内分支：0.388ms；按照 warp 分支：0.199s。因为一个 warp 内的线程只能执行相同的任务。如果需要走到不同分支，执行就会串行化。
...
(a)~去掉 \texttt{\_\_syncthreads()} 后，线程可能在另一个 warp 尚未写入 \texttt{buf[255-t]} 时就读取它，产生读后写竞争；该屏障保证所有线程完成 shared memory 写入后才开始读取。(b)~$t$ 与 $255-t$ 所在的 warp 分别为 $\lfloor t/32\rfloor$ 和 $7-\lfloor t/32\rfloor$，不可能相同；部分位置持续正确只是因为当前 warp 调度次序较稳定，后读取的一方恰好看到了已完成的写入，并无正确性保证。

\prob{3.4}{CONCEPT}
\texttt{\_\_syncthreads} 只能同步本 block 内的 threads，那需要全 grid 同步时，标准做法是什么？

标准做法是把同步点前后的工作拆成两个 kernel，并在同一 stream 中依次启动；前一个 kernel 完成后后一个才会执行，因此 kernel 边界构成全 grid 的同步点。若 Host 也必须等待结果，再调用 \texttt{cudaDeviceSynchronize()}；cooperative groups 的 grid 级同步仅适用于满足协作启动等限制的特殊场景。
...
实测（RTX 5090，复跑 3 次稳定）：interleaved 0.0062 ms、contiguous 0.0062 ms，比值 1.00x；shuffle 版 0.0043--0.0062 ms。判测提示两版几乎一样（不影响 PASS）；判测给出的参考是 A100 2.22x、V100 2.33x。

为什么这里测不出差距？理论上 interleaved 应该更慢：它每轮干活的 lane 隔 $2s$ 取一个（\texttt{tid \% (2*s) == 0}）、散布在全部 8 个 warp 里，每轮每个 warp 都要照常取指发射、多数 lane 空转，还要对运行期步长取模；contiguous 的活跃线程挤在低编号一端（\texttt{tid < s}）、活跃 warp 数每轮减半（$s=128$ 时 4 个、$s=64$ 时 2 个、$s \le 32$ 后只剩 1 个），配对元素相邻访存，浪费的发射槽位少得多。但本题 kernel 极小：4096 blocks $\times$ 256 线程、每 block 仅 8 轮配对，单次 launch 约 6 $\mu$s，其中 kernel 启动、每轮两次 block 级 \texttt{\_\_syncthreads}、首尾访存等固定开销远大于分支差异，三个 kernel 都压在同一个开销地板上，cudaEvent 四位小数的精度分辨不出差别。divergence 的代价本身依然存在（prob 3.2 在计算量更大的实验里测到了接近 2 倍的差距），只是在这个规模下被固定开销淹没。shuffle 版把最后 32 个元素换成 warp 内寄存器级交换、省掉多轮 \texttt{\_\_syncthreads}，实测与前两版持平或略快。
...
\begin{center}\small
  \begin{tabular}{@{}lp{8em}p{7em}p{6em}p{8em}@{}}
    ...
    global        & 整个 grid           & 应用程序  & 片外       & 程序员 \\
    constant      & 整个 grid（只读）       & 应用程序  & 片外（片上缓存） & 程序员 \\
    L1 / L2 cache & L1：单个 SM；L2：全 GPU & 硬件决定  & 片上       & 硬件  \\
    \bottomrule
  \end{tabular}
\end{center}
...
Constant cache 最适合一个 warp 内所有线程读取同一地址的访问模式，此时一次读取可以广播给整个 warp。本题所有线程在同一轮都读取相同的COEF[k]，符合该模式；但系数表仅有 32 字节，global 版本也很容易命中缓存，而且 kernel 还包含全局数组读写和依赖的浮点运算，因此 constant memory 未必带来可测的提速。若同一 warp 读取多个不同 constant 地址，请求会按不同地址拆分，优势会减弱。

\prob{4.4}{CONCEPT}
判断对错，可以顺带补一句理由。
\begin{enumerate}[label=(\alph*)]
  \item local memory 的“local”指作用域私有，它实际上在片外显存里。（对）
  \item 对数组用运行期才知道的下标做索引，可能迫使它被放进 local memory。（对）
\end{enumerate}
...
实测（RTX 5090）：naive 2.5282 ms（6.64 GB/s）、priv 0.0145 ms（1158.19 GB/s），提速 174.53x（参考：A100 实测 149x、V100 实测 86x）。

提速来源：naive 版把 16.7M 次 atomicAdd 全部打到全局的 256 个计数器上——全 grid 的线程同时挤这 256 个地址，每次 atomic 都要经过 L2 的串行化并往返显存，吞吐被全局 atomic 单元卡死。priv 版先把竞争拆到每个 block 自己的 shared memory 计数器里（片上、低延迟，竞争范围只剩本 block 的 256 个线程），同步后每个 block 只对全局做 256 次汇入，全局 atomic 总数从 16.7M 降到 $1024 \times 256 = 262144$ 次，且不同 block 大概率汇入不同 bin，跨 block 冲突也少。对全局内存的 atomic 流量下降两个数量级，这是提速的主要来源。
...
\begin{center}\small
  \begin{tabular}{@{}lcccccc@{}}
    ...
    \midrule
    GB/s   & 1899.5 & 1187.4 & 733.6 & 542.2 & 628.3 & 899.9 \\
    \bottomrule
  \end{tabular}
\end{center}

带宽从 stride 1 到 8 明显下降，因为 warp 内相邻线程访问的地址逐渐分散，
一次内存事务中真正使用的数据比例降低，访存合并效果变差。stride 继续增大
时带宽反而回升，这是因为 $n$ 和 stride 都是 2 的幂：stride 为 $s$ 时，
映射 $(i\times s)\bmod n$ 只会访问 $n/s$ 个不同输入，每个输入被重复读取
$s$ 次。输入工作集因此从 64 MiB 逐渐缩小到 2 MiB，增强的缓存复用开始
抵消非合并访问的代价，所以结果并不单调下降。
...
设备为 NVIDIA GeForce RTX 5090，每个 SM 有 100 KB shared memory，
每个 SM 最多常驻 1536 个线程。
...
\begin{center}\small
  \begin{tabular}{@{}lcccccc@{}}
    ...
    实测带宽 (GB/s)                & 1575.1  & 1573.2  & 1574.5  & 1575.7 & 1578.9 & 824.3  \\
    \bottomrule
  \end{tabular}
\end{center}

\texttt{cudaOccupancyMaxPotentialBlockSize} 在 shared memory 为 0 时建议
block size 为 768。
...
\begin{enumerate}[label=(\alph*)]
  \item 以每 block 使用 29.0 KB shared memory 为例，共享内存限制给出的
        驻留数为 $\lfloor 100/29\rfloor=3$，线程数限制给出的驻留数为
        $\lfloor 1536/256\rfloor=6$，所以最终每个 SM 可驻留 3 个 block。
        occupancy 为 $3\times256/1536=50\%$，与 API 的结果一致。
  \item global memory 访问具有较高延迟，一个 warp 等待数据时，SM 需要
        切换到其他常驻 warp 执行。occupancy 降低意味着可切换的 warp 和在途
        内存请求变少；当数量不足以隐藏延迟、跑满内存系统时，带宽就会下降。
        不过 50\% occupancy 时已有足够多的 warp，因此带宽仍处于饱和平台；
        降到 16.7\% 后才出现明显损失。
  \item 这块卡的档位与题目中的示例不同，而且结果并非严格单调。由
        100\% 降至 83.3\% 时，带宽从 1575.1 GB/s 变为 1575.7 GB/s，变化
        仅约 $0.04\%$，属于测量波动；由 50\% 降至 16.7\% 时，带宽从
        1578.9 GB/s 降至 824.3 GB/s，下降 754.6 GB/s，即约 $47.8\%$。
        occupancy 超过隐藏延迟所需的阈值后，继续增加并不会提高已经饱和的
        带宽；低于该阈值后，在途请求不足，性能才会快速下降，因此二者不成
        正比。
\end{enumerate}
...
实测 host 不等待 GPU 为 0.0024 ms，host 等待 GPU 为 0.5576 ms，
\texttt{cudaEvent} 计时为 0.5509 ms。

\begin{enumerate}[label=(\alph*)]
  \item 应将 \texttt{cudaEvent} 测得的 0.5509 ms 作为 kernel 耗时。
        两个 event 位于同一 stream，记录的是 GPU 时间线上夹在它们之间的
        kernel 执行时间。
  \item host 不等待 GPU 的 0.0024 ms 只测到了 host 提交异步 kernel
        launch 并返回所需的时间，kernel 此时通常还没有执行完。host 等待 GPU
        的 0.5576 ms 是从提交 kernel 到 \texttt{cudaDeviceSynchronize} 返回的
        端到端墙钟时间，除 kernel 执行外还包含 host launch、同步调用及等待等
        开销，因此略大于 \texttt{cudaEvent} 的结果。
\end{enumerate}

\prob{5.2}{CONCEPT}
判断对错，可以顺带补一句理由。
\begin{enumerate}[label=(\alph*)]
  \item 同一个 stream 里的操作按提交顺序执行。（对）
  \item kernel 启动后，host 代码立刻继续往下执行。（对）
  \item unified memory 下，CPU 访问一页正被 GPU 占用的内存，会触发缺页与页迁移。（对）
\end{enumerate}
...
判断对错，可以顺带补一句理由。
\begin{enumerate}[label=(\alph*)]
  \item tile 是显存里的一块可变区域，kernel 通过指针直接改写它。（错）
  \item 对 tile 的一次运算（如两个 tile 相加）由编译器映射到 block 内的多个线程上执行。（对）
  \item tile 模型与 SIMT 模型互斥，一个 CUDA 程序只能选一种。（错）
\end{enumerate}
...
\begin{center}\small
  \begin{tabularx}{\linewidth}{@{}p{4.2em}
    ...
    \midrule
    并行单位 & block 内的 thread
         & block
         & program（处理一个 tile）                                               \\
    编号   & \texttt{blockIdx}、\texttt{threadIdx}
         & \texttt{ct.bid(axis)}
         & \texttt{tl.program\_id}\newline\texttt{(axis)}                   \\
    数据分工 & 线程用全局下标划分元素
         & block 根据 bid 加载并处理一个或多个 tile
         & program 用 pid 和 \texttt{tl.arange} 构造一组元素下标                      \\
    边界处理 & 用 \texttt{if} 逐线程判断
         & load 选择 padding；store 丢弃越界写
         & 构造 mask，传给 \texttt{tl.load}/\texttt{tl.store}                    \\
    \bottomrule
  \end{tabularx}
\end{center}

cuTile 将边界语义放在 tile 的 load/store 上，不需要逐线程写 \texttt{if}；
但若越界 load 的值会参与有效结果，程序员仍需选择合适的 padding mode。

\prob{6.3}{CONCEPT}
仍看上面这段代码。(a)~“每个线程对应哪个/些元素”由谁决定？(b)~列出一些在 CUDA SIMT 版向量加法里一定会出现、这里完全没体现出的概念。

(a)~ 每个线程处理哪个或哪些元素”由 cuTile 编译器决定；程序员只指定 tile 大小，并用 bid 指定当前 block 处理哪个 tile。

(b)~ 这里没有显式出现 SIMT 版本中的 threadIdx、blockDim、线程全局下标计 a[i] 形式的逐元素访存，以及每个线程执行的越界 if。这些线程级细节都由器根据 tile 形状映射到底层硬件。
...
改动只有两处：kernel 内的计算表达式一行（\texttt{z = x * 2.0} 改为 \texttt{z = tl.maximum(x * a + b, 0.0)}），以及 kernel 签名与启动参数（把标量 \texttt{a}、\texttt{b} 传进去）。pid、offsets、mask、load/store、grid 计算这些主体代码一行未动。原因在于 Triton 的并行单位是 program（处理一个 tile），计算表达式直接写在 tile 向量上而不是逐线程的标量上：线程到数据的映射与边界处理已经由外围脚手架（pid + \texttt{tl.arange} + mask）包办，与具体计算公式无关，所以改语义只需动 tile 级的那一行。而 Module 2 的 CUDA SIMT 版里，公式和逐线程下标运算写在一起，改动要顺着线程视角重写（对照 Guide 1.2.2.3 的 tile 视角）。
...
grid 尺寸仍有直接对应：空 1 里 \texttt{T.ceildiv(N, block\_N)} 与 \texttt{T.ceildiv(M, block\_M)} 就是 2.6 空 4 两个方向向上取整的翻版。行号、列号只剩半个对应：\texttt{by * block\_M}、\texttt{bx * block\_N} 给出 tile 的起点坐标（空 2、空 3 里要自己写），而 tile 内每个元素具体由哪个线程处理已交给编译器（\texttt{T.copy}/\texttt{T.Parallel}），逐线程的 row/col 计算消失了。完全没有对应的是边界保护：它被 \texttt{T.copy} 的越界语义吃掉——搬进 shared 时自动填充越界部分、写回时自动丢弃越界写，所以不用再像 2.6 空 3 那样手写 \texttt{if (row < M \&\& col < N)}。

\prob{7.5}{CONCEPT}
补全下表，每个空填“用户”或“编译器”（二者都涉及的要写清楚各自的范围）。

\begin{center}\small
  \begin{tabularx}{\linewidth}{@{}p{7.5em}
    >{\raggedright\arraybackslash}X
    >{\raggedright\arraybackslash}X
    >{\raggedright\arraybackslash}X
    >{\raggedright\arraybackslash}X@{}}
    ...
    线程到数据的映射
        & 用户
        & 编译器（用户指定 block 对应的 tile）
        & 编译器（用户指定 program 对应的 tile）
        & 编译器（用户给出 tile 与并行迭代域）                                            \\
    边界处理
        & 用户
        & 二者：编译器处理越界，用户选择 load 的 padding
        & 用户（显式构造 mask）
        & 二者：\texttt{T.copy} 由编译器处理，显式索引由用户判断                              \\
    tile / block 尺寸的选择
        & 用户
        & 用户
        & 用户
...
因为 Triton 把这些全部收进了编译器。对照 Bonus 的 \texttt{matmul\_triton.py}：它只写 tile 级的 \texttt{tl.load}、\texttt{tl.dot} 和沿 K 的循环，shared memory 分级缓存、软件流水、累加器在寄存器里的布局都由 Triton 编译器自动安排，用户能调的旋钮只剩 BLOCK 大小（以及 \texttt{num\_warps} 这类启动参数）。而 TileLang 把存储层次和调度结构当作语言构件显式暴露出来：shared tile（\texttt{T.alloc\_shared}）、寄存器 fragment（\texttt{T.alloc\_fragment}）、显式搬运（\texttt{T.copy}）、流水结构（\texttt{T.Pipelined} + \texttt{num\_stages}）都要自己写。这是 prob 7.5 表格之外的另一条分界线：Triton 隐藏了内存层次，TileLang 把它显式化——代价是要填的空更多，好处是每一级存储和每一段流水都看得见、可控制（也便于 Bonus 里逐项调参）。
...
对比 7.7 与 7.8，归约、边界处理、按形状编译这三项其实都是用户显式处理的，区别只在“显式”的写法。归约：Triton 里写 \texttt{tl.max}/\texttt{tl.sum}（axis=0），TileLang 里写 \texttt{T.reduce\_max}/\texttt{T.reduce\_sum}（dim=0），两者都要求用户指明归约轴，编译器只负责把归约实现映射到 warp/block 的并行结构上。边界处理：Triton 用 \texttt{mask=mask, other=-inf} 传给 load/store；TileLang 用 \texttt{T.if\_then\_else(j < N, X[row, j], -inf)} 填充无效元素、最后再用 \texttt{if j < N} 守卫 store——两边都是用户手写，并非 cuTile 那种由 load/store 语义隐式处理的模式。按形状编译：两者都由 JIT 编译器隐式完成——Triton 按 \texttt{BLOCK\_SIZE} 这个 constexpr 特化并缓存，TileLang 的 \texttt{@jit} 按 (\texttt{M}, \texttt{N}, \texttt{BLOCK\_N}) 参数组合缓存；用户只负责算出 \texttt{next\_power\_of\_2(N)} 并传进去。真正被两边编译器共同接管的，是线程到数据的映射（\texttt{tl.arange} 构造 offsets vs \texttt{T.Parallel}/\texttt{T.Kernel} 的隐式划分），这一层在两种写法里都看不见 thread。
...
判断对错，可以顺带补一句理由。
\begin{enumerate}[label=(\alph*)]
  \item PTX 是 GPU 直接执行的机器码。（错）
  \item 只嵌入了 \texttt{sm\_70} SASS 的可执行文件，能在 compute capability 9.0 的卡上运行。（错）
  \item 一个 fatbin 可以同时携带多个架构的 SASS 和 PTX。（对）
  \item JIT 编译由驱动在运行时完成。（对）
\end{enumerate}

PTX 是虚拟 ISA（中间表示），GPU 直接执行的是 SASS；PTX 要经 \texttt{ptxas} 离线编译、或由驱动在运行时 JIT 成 SASS 后才能执行（Guide 2.1.1）。SASS 与具体 compute capability 绑定，只含 \texttt{sm\_70} SASS 的可执行文件无法在 9.0 的卡上运行，需要 fatbin 携带对应架构的 SASS，或用 PTX 兜底。fatbin 本身是容器，可以同时打包多个架构的 SASS 和 PTX，运行时从中挑选；JIT 这一步由驱动完成（Guide 1.3.2）。
...
(a) 实测报错为 \texttt{CUDA error cudaErrorNoKernelImageForDevice at m0\_env/01\_hello.cu:11: no kernel image is available for execution on the device}（error 209，exit code 1）。可执行文件里只有 \texttt{sm\_90} 的 SASS，而 5090 是 compute capability 12.0——SASS 与具体架构绑定，fatbin 里既没有匹配的 image 也没有 PTX 兜底，运行时无从加载。

(b) 实测能正常运行（exit code 0，4 个 block 全部打印 hello）。这个版本只带 \texttt{compute\_75} 的 PTX，驱动在首次加载时把它 JIT 编译成 \texttt{sm\_120} 的 SASS 再执行。即 PTX 到机器码的编译发生在运行时、由 CUDA driver 完成（Guide 1.3.2、2.1.1）。

\prob[Optional]{8.3}{CONCEPT}
请简单说明 Runtime API 与 Driver API 各自的定位。\texttt{cudaMalloc} 属于哪个？

Runtime API（\texttt{cuda\_runtime.h}，libcudart）是 Driver API 之上的高层封装：隐式完成设备初始化与 primary context 管理，提供模板化的 kernel 启动语法以及事件、流等便利设施，面向常规应用程序开发。Driver API（\texttt{cuda.h}，libcuda）贴近驱动、粒度更细：\texttt{cuInit}/\texttt{cuCtxCreate}/\texttt{cuModuleLoad} 等都要显式调用，能精细控制多设备、多上下文、模块加载与 JIT，面向库与工具链开发者（CUDA runtime 本身也构建在它之上，Guide 1.3）。\texttt{cudaMalloc} 属于 Runtime API，对应 Driver API 的 \texttt{cuMemAlloc}。
...
\end{lstlisting}
        ...
        实测（RTX 5090）：BS=8 → 0.397 ms / 5409.8 GFLOPS，BS=16 → 0.317 ms / 6776.9 GFLOPS，BS=32 → 0.363 ms / 5912.2 GFLOPS。
...
\end{lstlisting}
        ...
        实测（RTX 5090）：(32,32,32) 98.9 TFLOPS，(64,64,32) 157.2，(128,64,32) 160.6，(128,128,32) 163.7，(128,128,64) 144.8，(64,64,64) 174.5；cuBLAS 167.7 TFLOPS。最优 BLOCK=(64,64,64)，约 174.5 TFLOPS。
...
\end{lstlisting}
        ...
        实测（RTX 5090）：(64,64,32,threads=128,stages=1) 89.5 TFLOPS，(64,64,32,128,3) 131.0，(128,128,32,128,3) 150.9，(128,128,64,256,3) 164.5；最优 (128,128,64,256,3) 约 164.5 TFLOPS。最大配置 (128,256,64,256,3) 启动失败：\texttt{Failed to set the allowed dynamic shared memory size to 147456}——3 级流水的两块 shared tile 共需 144 KB，超过 5090 每 block 的 shared 上限（99 KB，opt-in 后 101376 B），这个 tile 组合在该卡上放不下。
\end{enumerate}

性能差距的原因（本次实测均在 RTX 5090 上；naive CUDA 为 fp32 的 $1024^3$，Triton / TileLang / cuBLAS 为 fp16 输入、fp32 累加的 $2048^3$）：

naive CUDA 只有 5.4--6.8 TFLOPS：每个输出元素要把 A 的一行、B 的一列从 global memory 反复读 K 遍，算术强度极低、纯带宽受限，且完全用不到 tensor core；BS 只改线程块形状、不改变数据复用结构，BS=8/16/32 实测只差两成左右（最优 BS=16，6776.9 GFLOPS），印证瓶颈在访存而不在线程组织。Triton 与 TileLang 把数据按 tile 搬进 shared memory 复用、用 fp16 tensor core 执行 \texttt{tl.dot}/\texttt{T.gemm}，达到 150--175 TFLOPS，比 naive 高约 25 倍（A100 上这个对比更悬殊，见上方注）。cuBLAS 实测 167.7 TFLOPS，与 Triton 最优配置（(64,64,64)，174.5 TFLOPS）在同一水平——在这个规模上 Triton 自动生成的代码已经接近手写调优的库。

TileLang 控制粒度更细却略慢（最优 164.5 TFLOPS vs Triton 174.5）：细粒度不等于自动变快。TileLang 把 layout、shared 分配、流水结构显式交给用户，但性能最终取决于编译器为这些结构生成的代码质量（layout inference、指令调度、tensor core 指令选择、流水展开）以及对目标架构的调优成熟度；本仓库固定的 tilelang 0.1.12 仍在快速迭代，这些后端优化不如更成熟的 Triton。一个更直观的例子是最大配置 (128,256,64,256,3) 直接启动失败：3 级流水需要 144 KB shared，超过 5090 每 block 99 KB 的上限——tile 尺寸与流水级数共同决定 shared 用量，在 TileLang 里这笔资源账要用户自己算，这正是“控制粒度更细”的另一面。
