...
\textcolor{gray}{（待补：interleaved / contiguous 实测耗时与比值，及性能差距解释）}
...
\textcolor{gray}{（待补：naive / priv 实测耗时与提速比，及提速来源的解释）}
...
\textcolor{gray}{（待补：(a) sass-only 版本的报错信息；(b) ptx-only 版本能否运行，以及 PTX 在何时、由谁编译成该卡的机器码）}
...
\end{lstlisting}
        ...
        \textcolor{gray}{（待补：BS=8/16/32 实测数据）}
...
\end{lstlisting}
        ...
        \textcolor{gray}{（待补：各 tile 配置实测数据）}
...
\end{lstlisting}
        ...
        \textcolor{gray}{（待补：各 block\_M/block\_N/block\_K/num\_stages 配置实测数据）}
\end{enumerate}

\textcolor{gray}{（待补：三组实测数据对比与性能差距分析）}
